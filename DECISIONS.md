...
| **Last change** | Phase 5 commit 10 (close-out): full regression suite green (454 passed, 3 skipped), all BUILD.md Phase 5 exit criteria confirmed met except frontend checkpoint 2 (N/A, solo project). Manual verifier review performed against real dev-case documents with a real LLM — `gemini-3.5-flash-lite`, not the pinned `gemini-3.5-flash` (E9), because the pinned model's free-tier quota (confirmed 20 requests/day/model) was already exhausted; E9 itself is unchanged. Three scenarios reviewed (fully-supported case, genuine-abstention case, synthetic conflicting-evidence case): verifier reasoning was specific to each field's actual evidence text in all three, correctly distinguishing supported values, correct abstentions, and cross-document conflict. re_retrieve was not observed in any real run — that path remains verified only by the stubbed test suite. See §5 Phase 5 and §6 for full detail. |
...
| Metric | Value | Date |
|--------|-------|------|
| Full regression suite | **454 passed, 3 skipped, 0 failed.** Ruff clean; mypy clean in CI mode (62 source files) and `--strict` on every Phase 5 module (`app/orchestration/`, its tests, `app/api/cases.py` — 14 files). | 2026-07-29 |
| Exit criteria (`BUILD.md` Phase 5) | All conditional edges exercised by deterministic tests — met (`tests/orchestration/test_edges.py`, `test_conflict.py`). Retry bound enforced — met, exact-count proof (`RETRY_BUDGET + 1` extraction attempts, never more). Resume-after-crash test green — met (`tests/orchestration/test_checkpoint.py`, resumption proven via a freshly-constructed `CompiledStateGraph`, not object identity). Traces persisted for every verifier decision — met, from the verifier node's first commit. Frontend integration checkpoint 2 — **N/A**, solo project, no separate frontend developer exists; same collapse already recorded for R11/R12 (§6, 2026-07-26). | 2026-07-29 |
| Manual verifier review — model actually used | **`gemini-3.5-flash-lite`, not the pinned `gemini-3.5-flash` (E9).** The pinned model's `generate_content_free_tier_requests` quota returned `429 RESOURCE_EXHAUSTED` on the very first live verifier call and again on a second attempt after a delay; the error payload's `quotaId` — `GenerateRequestsPerDayPerProjectPerModel-FreeTier`, `quotaValue: 20` — makes this a confirmed **20-requests-per-day cap per model**, a more precise characterization than Phase 4's "far more restrictive than a simple per-minute ceiling." Gemini quotas are tracked per model (`quotaDimensions.model` in the error payload), so `gemini-3.5-flash-lite` — verified working during E9's own pinning decision — had a separate, unexhausted bucket and was used for this diagnostic review only. `DECISIONS.md` E9 is **unchanged**; no production code was modified to make this substitution (a one-off script outside the repo overrode `app.extraction.llm_client.GENERATION_MODEL` for its own process only). Findings below describe the lite variant's behavior, not the pinned model's — stated plainly so this is not mistaken for a measurement of E9 itself. | 2026-07-29 |

**Methodology.** Three real-LLM scenarios, run through the actual compiled graph (`app.orchestration.graph.run_graph`, `privacy_mode=NONE`) with real retrieval (`gemini-embedding-001`, E8) and real generation — no stubs anywhere in the call path. `privacy_mode=NONE` was deliberate: it isolates verifier reasoning quality from the already-documented, unrelated Phase 3/4 Tokenize entity-type gap (`NAME`/`ADDRESS`/`EMAIL`/`DATE` have no FF1 alphabet), which would otherwise make every field fail before the verifier ever ran, exactly as it did in the Phase 4 measurement above. Not committed to the repository (BUILD.md Phase 5 commit 10 calls for a manual review, not a new harness script); output was read directly and is transcribed below, not replayed from a stored artifact.

1. **`kyc_complete_1`** (single document, all 10 fields genuinely present). Every field: extracted correctly, verifier decision `accept`, reasoning specific to that field's exact evidence text (e.g. citing "Existing Bank Account Number: 987654321012" verbatim for `linked_account_number`) rather than generic boilerplate.
2. **`kyc_missing_optional_fields_1`** (3 of 10 fields genuinely absent — `email_address`, `linked_account_number`, `annual_income`). All 7 present fields: `accept`, evidence-specific reasoning as above. All 3 genuinely-absent fields: extractor correctly abstained, and the verifier separately judged the *abstention itself* correct — e.g. for `linked_account_number`, reasoning read "The evidence chunk explicitly states 'No existing bank account.', confirming that there is no account number present. The extractor correctly abstained... which is directly and unambiguously supported by the text." This is the strongest single observation in this review: the reasoning names the specific sentence that justifies the abstention, differently worded per field, not a template response to "value is null." Separately, and worth flagging for Phase 6/7's methodology: `date_of_birth`'s extracted value was `05/08/1992`, a reformatting of the source text's `05 August 1992` — not an exact string match to the case's own ground truth field, yet the verifier `accept`ed it as "directly supported." The verifier assesses evidentiary support, not string-exact accuracy against a ground-truth label it never sees; a downstream accuracy scorer doing naive string comparison could disagree with a verifier `accept` on a case like this one.
3. **Synthetic conflict** (purpose-built, not a dev-case fixture — one field, two documents stating different dates of birth for the same field, mirroring `tests/orchestration/test_conflict.py`'s scenario). Extractor abstained rather than arbitrarily picking one of the two values (not validated as general extractor behavior — a single observation). Verifier: `escalate`, with reasoning that correctly named both source documents and both conflicting values by name ("Chunk 0 (aadhaar.pdf) states... 22/09/1987, while Chunk 1 (passport.pdf) states... 14/06/1985... human intervention is required").

**What this review does and does not support.** All three scenarios landed on `accept` or `escalate` — genuinely useful, specific reasoning was observed for both, including correctly distinguishing "evidence supports this value" from "evidence supports abstaining" from "evidence conflicts." This is measured evidence against BUILD.md's own named risk ("Verifier is an if-statement in disguise") for the scenarios tested: the reasoning text is demonstrably conditioned on the specific evidence content, not a fixed template keyed only on the decision. **What was not observed, and should not be assumed:** no `re_retrieve` decision occurred in any of the three real runs (all evidence was either clearly sufficient or clearly conflicting, never ambiguous-in-between), so the retry path's real-model behavior remains unmeasured — the automated suite (`tests/orchestration/test_edges.py`) covers that path only with a stubbed decision. `full_tokenize`/`policy_engine` modes were not exercised with a real verifier call at all (out of scope for this review, given the already-known Tokenize gap above). Three scenarios is a spot check, not a statistically meaningful sample — the calibrated verifier audit BUILD.md actually specifies (10–15 ambiguous cases, precision/recall against ground truth) is explicitly Phase 6/7 work (`ARCH §8`, `DECISIONS.md` V11/V12), not this commit's job. This review's conclusion is narrow and honest: the mechanism reasons over evidence rather than thresholding a number, for the cases tried; it is not a claim about accuracy, calibration, or behavior under ambiguity.
...
| 2026-07-29 | R16 | *(absent)* | Added: LangGraph `InMemorySaver`, `thread_id = case_id` | Phase 5 commit 8 (BUILD P5 task 8). Closes C1 (§7): "crash-resume" is scoped to an interrupted-but-still-in-process run, matching R8/R14's already-accepted process-memory-only trade-off rather than being in tension with it. A durable `langgraph-checkpoint-sqlite` backend was evaluated and rejected — it would give partial, misleading durability, since the pseudonym map (R8) and vector index (R14) are not durable either, and no `BUILD.md` task or CLAUDE §6 justification exists for the new dependency that backend would require. Verified empirically before relying on it: `InMemorySaver` persists partial state after a mid-run node failure, and a *second, freshly-constructed* compiled graph built from the same saver instance and `thread_id` correctly resumes from that checkpoint without reprocessing already-completed fields — proving resumption is keyed by `thread_id`, not Python object identity. | None — no measurements taken yet |
| 2026-07-29 | §5 Phase 5 close-out | `*pending*` | Full regression green (454 passed, 3 skipped); manual verifier review performed (see §5 Phase 5) | Phase 5 commit 10 (BUILD P5 task 10/exit criteria). E9 (pinned model, `gemini-3.5-flash`) itself is **unchanged** — the manual verifier review used `gemini-3.5-flash-lite` instead, transparently, because E9's free-tier quota was exhausted before the review could run against it (confirmed, not assumed: `quotaId: GenerateRequestsPerDayPerProjectPerModel-FreeTier`, `quotaValue: 20` — a 20-requests-per-day-per-model cap, more precise than Phase 4's own characterization). This refines the *understanding* of E9's quota constraint; it does not change E9's value, so no prior E9-dependent measurement needs re-running. | None — E9's value is unchanged, only its documented quota behavior is more precisely characterized |
...
|---|--------------|--------------------|
| C1 | ~~Crash-resume is required to reproduce identical state, but the session pseudonym map is in process memory and does not survive a restart~~ — **Resolved 2026-07-29**: "crash-resume" is scoped to an interrupted-but-still-in-process run (R16, §4), consistent with the pseudonym map's own already-accepted process-memory-only trade-off rather than in tension with it. Real cross-process durability was deliberately not pursued — see R16 for why. | ARCH §11.7 vs. BUILD P5 |
