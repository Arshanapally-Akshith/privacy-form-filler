...
| **Last change** | Phase 2 tasks 1-2: form schema format pinned and the two `R9` schemas authored. Phase 2 task 3: extraction node added; E9 (generative LLM) pinned to `gemini-3.5-flash` after live verification against the project's Gemini account. Phase 2 task 4: E7 (confidence score definition) pinned; see §6 |
...
E6 is unresolved by design. Do not infer it; pin it in the stated phase and record it here
in the same commit.
